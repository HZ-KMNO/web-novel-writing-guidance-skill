...
![网文小说写作指导skill 徽标](assets/logo.png)

这是一个面向 AI agents 的中文小说/网文写作指导 skill，用于指导 AI 作为长期创作项目的写作主脑工作，而不是一次性文本生成器。

它包含我们之前讨论过的核心流程：用户提供创意、建议和上下文，AI 选择相关资料与 skills，先生成章节章纲与关键信息文件，再整理章节任务卡，随后写 A 稿；用户反馈后修改为 B 稿；用户满意后进行去 AI 味，生成 C 稿；最后更新连续性记录、伏笔、人物变化和下一章承接点。
...
```text
使用 网文小说写作指导skill。我会给你创意和上一章概要，请先生成章节章纲与关键信息文件，再整理章节任务卡，随后写 A 稿。
```

```text
使用 网文小说写作指导skill，根据我的修改意见先分类反馈，再把这一章改成 B 稿。保留我满意的部分。
```

```text
内容方向已经确认。使用 网文小说写作指导skill 做 C 稿去 AI 味，不要改设定和情节结果，最后输出连续性记录。
```
